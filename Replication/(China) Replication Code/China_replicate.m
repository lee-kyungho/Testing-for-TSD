...
dummy_provgb = dummyvar(categorical(sample_data.provgb));
dummy_quarter = dummyvar(categorical(sample_data.quarter));
...
y = table2array(sample_data(:,['visib']));
X = [ones(size(sample_data,1),1), table2array(sample_data(:,['wdsp' "temp" "prcp"])), dummy_provgb(:,1:end-1), dummy_quarter(:,1:end-1)];

beta_hat = (X'*X)\X'*y;

X_0 = [ones(size(sample_data0,1),1), table2array(sample_data0(:,['wdsp' "temp" "prcp"])), dummy_provgb(1:size(sample_data0,1),1:end-1), dummy_quarter(1:size(sample_data0,1),1:end-1)];
X_1 = [ones(size(sample_data1,1),1), table2array(sample_data1(:,['wdsp' "temp" "prcp"])), dummy_provgb(1:size(sample_data1,1),1:end-1), dummy_quarter(1:size(sample_data1,1),1:end-1)];


% Residuals
res_visib_data_0 = sample_data0.visib - X_0*beta_hat;
res_visib_data_1 = sample_data1.visib - X_1*beta_hat;

sample_data0.res = res_visib_data_0;
sample_data1.res = res_visib_data_1;

% Reshaping data set which fits well with Path-wise bootstrap
...
covariate_0 = cat(3,  wdsp_test_data_0, temp_test_data_0, ...
                         prcp_test_data_0);
covariate_0_cg = cat(3,provgb_test_data_0, quarter_test_data_0);

covariate_1 = cat(3,  wdsp_test_data_1, temp_test_data_1, ...
                         prcp_test_data_1);
covariate_1_cg = cat(3, provgb_test_data_1, quarter_test_data_1);
end
...
if residual_flag == 1
    ...
    sample1_covariate_cg = covariate_0_cg;
    sample2_covariate = covariate_1;
    sample2_covariate_cg = covariate_1_cg;
else
...
if residual_flag == 1
    ...
    sample1_covariate_cg = covariate_1_cg;

    sample2_covariate = covariate_0;
    sample2_covariate_cg = covariate_0_cg;
...
phi_dist_11_max_1_list = [];
phi_dist_12_max_1_list = [];
phi_dist_21_max_1_list = [];
phi_dist_22_max_1_list = [];
...
Lamb_22_max = Lambda(D_22_collection,p,'sum'); % Output dim: ngrid * 1
...
prov_dummy_b = dummyvar(categorical(covr_cg_b(:,1)));
quarter_dummy_b = dummyvar(categorical(covr_cg_b(:,2)));
Thai_array_b = [y_b, covr_b, prov_dummy_b, quarter_dummy_b];
