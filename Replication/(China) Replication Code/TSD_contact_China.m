function [T_11_max_contact, T_12_max_contact, ...
          T_21_max_contact, T_22_max_contact] = ...
...
for i = 1:size(N_subset_colletion_11,2)
    ...
    B_11 = contact_set_estimation(r_N,D_11,A,c_n_11); % censoring is in the function
    B_12 = contact_set_estimation(r_N,D_12,A,c_n_12); 
    B_21 = contact_set_estimation(r_N,D_21,A,c_n_21); 
           
    % -------- Bootstrap Lambda
    
    Lamb_11_max_A = Lambda(b_D_11_A_recentered,p,'max'); % Output dim: ngrid * 1 * b
    Lamb_12_max_A = Lambda(b_D_12_A_recentered,p,'max'); % Output dim: ngrid * 1 * b
    Lamb_21_max_A = Lambda(b_D_21_A_recentered,p,'max'); % Output dim: ngrid * 1 * b

    %---- Integration for Bootstrap version of T_{N} %
    % In here, we take integration on "Contact" set
    
    T_11_max_contact = T_11_max_contact + r_N^p * trapz(Lamb_11_max_A(B_11,:,:),1);
    T_12_max_contact = T_12_max_contact + r_N^p * trapz(Lamb_12_max_A(B_12,:,:),1);
    T_21_max_contact = T_21_max_contact + r_N^p * trapz(Lamb_21_max_A(B_21,:,:),1);
...
for i = 1:size(N_subset_colletion_22,2)
    ...
    B_22 = contact_set_estimation(r_N,D_22_collection,A,c_n_22); % Use a collection for considering the terminal period
    
    b_D_22_collection_A_recentered = b_D_22_collection_recentered;       
    b_D_22_collection_A_recentered(:,A_complement,:) = 0;
        
    Lamb_22_max_A = Lambda(b_D_22_collection_A_recentered,p,'max'); % Output dim: ngrid * 1 * b
    
    %---- Integration for Bootstrap version of T_{N} %
    % In here, we take integration on "Contact" set
        
    T_22_max_contact = T_22_max_contact + r_N^p * trapz(Lamb_22_max_A(B_22,:,:),1);
